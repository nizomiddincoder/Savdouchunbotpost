...
async function buildReceipt(saleId) {
  const { rows } = await q(`SELECT s.*, sel.name AS seller_name FROM sales s JOIN sellers sel ON sel.id = s.seller_id WHERE s.id = $1`, [saleId]);
  const s = rows[0];
...
router.post('/', anyAuth, async (req, res, next) => {
  try {
    ...
    try {
      ...
      if (!c) {
        try {
          c = (await client.query('INSERT INTO customers (name, first_seller_id) VALUES ($1, $2) RETURNING id', [cname, req.user.id])).rows[0];
        } catch (e) {
      ...
      const sale = (await client.query(
        `INSERT INTO sales (seller_id, customer_id, customer_name, total_uzs, usd_rate)
         VALUES ($1, $2, $3, $4, $5) RETURNING id`,
        [req.user.id, c.id, cname, total, rate])).rows[0];
...
router.get('/', anyAuth, async (req, res, next) => {
  try {
    ...
    params.push(limit);
    const { rows } = await q(`
      SELECT v.id, v.created_at, v.customer_name, v.total_uzs, v.is_cancelled,
...
router.get('/my-stats', middleware('seller'), async (req, res, next) => {
  try {
    ...
    const recent = (await q(`SELECT v.id, v.created_at, v.customer_name, v.total_uzs, v.is_cancelled, v.cancelled_by, v.cancelled_at, v.cancel_reason,
             (SELECT count(*) FROM sale_items si WHERE si.sale_id = v.id) AS items_count
